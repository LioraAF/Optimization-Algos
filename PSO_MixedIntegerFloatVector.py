...
class ParticleSwarm(object):
    ...
    def run(self, seed=None):
        ...
        return self.x_best_g, self.f_best_g, diff, hist

def plot_hist(hist):
    plt.figure(figsize=(10, 6))
    plt.plot(hist, color='royalblue')
    plt.title("Histogram of Values", fontsize=16)
    plt.xlabel("Num Evaluations", fontsize=10)
...
if __name__ == "__main__":
    ...
    for index, funcName in enumerate(['genHcigar', 'genRotatedHellipse', 'genHadamardHellipse']):
        ...
        for i in range(Nruns):
            ...
            xbest, fbest, diff, hist = pso.run(seed=i)
